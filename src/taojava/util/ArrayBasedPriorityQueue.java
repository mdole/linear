...
public class ArrayBasedPriorityQueue<T> implements PriorityQueue<T> {
    ...
    /**
     * Create a new priority queue that holds up to capacity elements and uses
	...
	for (int i = 0; i < size; i++) {
	    if (this.order.compare(values[highestIndex], values[i]) > 0) {
		highestIndex = i;
	    }// if
	}// for
	T temp = values[highestIndex];
	values[highestIndex] = values[size - 1];
	size--;
	return temp;
    } // get(T)
